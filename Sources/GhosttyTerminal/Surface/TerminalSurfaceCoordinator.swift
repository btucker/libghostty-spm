...
final class TerminalSurfaceCoordinator {
    ...
    /// `synchronizeMetrics()` resets the gate so resizes and rotations render
    /// immediately.
    var renderPace: TerminalRenderPace = .full {
        didSet {
            guard renderPace != oldValue else { return }
    ...
    private func scheduleTickIfNeeded() {
        ...
        case .full:
            delay = 0
        case let .reduced(interval):
            delay = max(0, lastTickTimestamp + max(0, interval) - now)
        }

        scheduledTickGeneration &+= 1
        let generation = scheduledTickGeneration
        tickScheduled = true
        let formattedDelay = String(format: "%.3f", delay)
        TerminalDebugLog.log(
            .lifecycle,
            "tick scheduled delay=\(formattedDelay)"
        )
        if delay > 0 {
            DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self] in
                self?.runScheduledTick(generation: generation)
            }
        } else {
    ...
    private func runScheduledTick(generation: Int) {
        guard generation == scheduledTickGeneration else { return }
        tickScheduled = false
